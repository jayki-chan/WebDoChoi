    ...
    public partial class Default : System.Web.UI.Page
    {
        ...
        private void UpdateCartQuantity(int productId, int quantity)
        {
            try
            {
                ...
                if (quantity <= 0)
                {
                    var removeResult = CartService.RemoveFromCart(userId, sessionId, productId);
                    if (removeResult.IsSuccess)
                    {
                        Session["CartSummary"] = removeResult.CartSummary;
                        LoadCartItems();
                        ShowSuccessMessage("Đã xóa sản phẩm khỏi giỏ hàng");
                    }
                    else
                    {
                        ShowErrorMessage(removeResult.Message);
                    }
                    return;
                }
        ...
        protected void rptCartItems_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                ShowErrorMessage("Có lỗi xảy ra khi cập nhật giỏ hàng");
            }
        }
